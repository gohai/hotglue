...
function hide_on_hover_alter_render_late($args)
{
	...
	if (!$args['edit']) {
		if (isset($obj['hide_on_hover-hide']) && $obj['hide_on_hover-hide'] == 'hide') {
			...
			$script .= tab(2).'$(document).bind(\'mousemove\', function(e) { var d = $(\'#'.str_replace('.', '\\\\.', $obj['name']).'\'); var o = d.offset(); if (o.left <= e.pageX && e.pageX <= o.left+d.width() && o.top <= e.pageY && e.pageY <= o.top+d.height()) { d.css(\'visibility\', \'hidden\'); } else { d.css(\'visibility\', \'visible\') }; });';
			$script .= tab().'});'.nl();
			$script .= '</script>'.nl();
			$html = $html.$script;
			return true;
